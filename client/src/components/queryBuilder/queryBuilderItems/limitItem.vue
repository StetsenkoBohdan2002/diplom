  ...
  <div class="query-item match">
    <div class="query-item-main">
      ...
      <v-text-field
        ...
        min="0"
      ></v-text-field>
      <v-btn class="clear-btn" icon="mdi-close" @click="clearLimitQuery">
      </v-btn>
      <v-alert
        v-if="!size"
        border
...
<script>
/**
 * Компонент, що відповідає за створення операції агрегування $limit та задання відповідних параметрів
 * @vue/component
 * @module limitItem
 */
export default {
  ...
  data() {
    return {
      ...
      operation: null,
      /**
       * Стан, що відповідає за значення size для операції агрегування $limit.
       * @type {number|null}
       */
      size: null,
    }
  ...
  /**
   * A method to initialize a component.
   */
  created() {
    if (this.query.size) {
  ...
  methods: {
     ...
     */
    changeLimitSize(newValue) {
      this.$emit('changeLimitSize', this.item.id, this.query.queryId, newValue)
    },
    /**
     * Функція, що очищає поля поточного запиту limit для повторного заповнення
     */
    clearLimitQuery() {
      this.size = null
      this.$emit('changeLimitSize', this.item.id, this.query.queryId, null)
    },
    /**
