  ...
  <div class="query-item match">
    <div class="query-item-main">
      ...
      <v-autocomplete
        ...
        multiple
      ></v-autocomplete>
      <v-btn class="clear-btn" icon="mdi-close" @click="clearProjectLables">
      </v-btn>
      <v-alert
        v-if="!labels"
        border
...
export default {
  ...
  data() {
    return {
      ...
      operation: null,
      /**
       * Стан, що відповідає за значення labels для операції агрегування $match, яке в свою чергу масив містить полів для виконання пошуку.
       * @type {Array|null}
       */
      labels: null,
      /**
  ...
  /**
   * Спостерігач за полем labels.
   * @type {Array|null}
   * @property {function} handler - Обробник зміни поля labels.
   */
  /**
  ...
  watch: {
    labels: function (newValue) {
      ...
      this.changeProjectLables(newValue)
    },
  ...
  methods: {
    ...
    /**
     * Функція, що очищає поля поточного запиту project для повторного заповнення
     */
    clearProjectLables() {
      this.labels = null
      this.$emit('changeProjectLables', this.item.id, this.query.queryId, null)
    },
    /**
